...
import PastRoomCard, { type PastRoom } from "@/components/PastRoomCard";
import RoomDeletePopup from "@/components/RoomDeletePopup";

export default function Home() {
  const { data: session } = useSession();
  const [mode, setMode] = useState<"initial" | "create" | "join">("initial");
  const [myRooms, setMyRooms] = useState<PastRoom[]>([]);
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch("/api/room/delete", {
        method: "DELETE",
        body: JSON.stringify({ roomId: roomToDelete }),
      });

      if (res.ok) {
        setMyRooms((prev) => prev.filter((room) => room.roomId !== roomToDelete));
      }
    } catch (error) {
      console.error("Failed to delete room:", error);
    } finally {
      setIsDeleting(false);
      setRoomToDelete(null); // Close the modal
    }
  };
        ...
        <div className="relative z-10 p-6">
          ...
          <nav className="max-w-6xl mx-auto flex justify-between items-center mb-16 pt-4">
            ...
            <div className="flex items-center gap-4">
              ...
              >
                <span key={isDark ? "dark" : "light"} className="theme-toggle-icon block">
              ...
              {session ? (
                ...
                <button
                  ...
                  className="bg-stone-900 dark:bg-zinc-100 text-white dark:text-zinc-950 hover:bg-stone-800 dark:hover:bg-white px-6 py-2.5 rounded-full font-bold text-sm transition-all shadow-lg dark:shadow-[0_0_20px_rgba(255,255,255,0.1)] hover:shadow-xl"
                >
                  Sign Up / Login
                </button>
              )}
